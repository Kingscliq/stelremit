...
export function Navbar({ connected, setConnected }: { connected: boolean; setConnected: (val: boolean) => void }) {
  return (
    <nav className="relative z-10 flex items-center justify-between py-5 px-10 border-b border-border-1 backdrop-blur-md bg-[rgba(8,13,26,0.6)]">
      ...
      <div className="flex items-center gap-3">
        ...
        <WalletButton connected={connected} onClick={() => setConnected(!connected)} />
      </div>
    </nav>
  );
}
